        ...
        >
          ...
          <!-- <template>
            <div class="container">
              <br/>
              <br/>
              <textarea disabled :style="{ width: width + 'px', height: height + 'px' }" v-if="nonImageData" v-model="nonImageData">
...
import {usePaginationStore} from  '../store/datasets-paginataion';
import Vue from 'vue';
...
async function  loadFiles(files: FileList, setError: (err: Error) => void) {
  ...
  try {
    ...
    // emit the method for the detail component
  } catch (error) {
...
async function  loadFiles_tmp(files: File[], setError: (err: Error) => void) {
...
let globalDisplayData = "";
let countTmp:int = 0;
function readNonImageDataFromFile(){

  let tmp:int = countTmp;
  countTmp++;

  return "this is a test text " + tmp;     
}


async function loadClinicalData(setError: (err: Error) => void)
{

  const folderPath = "C:/Demo/images";
  ...
  try {
    ...
    const response = await axios.get(APIUrl, {
        params: {
          ...
          page_number: usePaginationStore().getNextPage
        },
      });
    const data = response.data;

    const fileName = data.files[0] 
    const fileList = await getFile(folderPath, fileName);
    const files = fileList.flat();

 

    for (let i = 0; i < files.length; i++) {
     let tmpFile = new File([files[i]], files[i].name, { type: "nii.gz"});

     convertedFileList.push(tmpFile);      
    }

    loadFiles_tmp(convertedFileList, setError) 
...
async function getFile(folderPath:string, file_name: string): Promise<File[]> {
  ...
  const response = await axios.get(APIUrl,{
      ...
      responseType: 'blob' });
  const file = new File([response.data], file_name);
  return [file]
}
...
export default defineComponent({
  ...
  computed:{
    nonImageData(){
      try {
        //return "test";
        let text:string =  this.readNonImageDataFromFile();
        return text;
      } 
      catch (error) {
  ...
  methods:{
    ...
    startTimer() {
      ...
      stopTimer() {
        ...
        const timeDiff = this.endTime.getDate() - this.startTime.getDate();
        ...
        writeToExcel(this.startTime, this.endTime, timeTaken.toString(), this.selectedValue );
      },         
    invokeNonImagingDataRetreival(){
      this.$emit('getNonImagingData', 'test');
    }
  },
  setup(props: {}, { root }) {
    ...
    const paginataionStore = usePaginationStore();
    useGlobalErrorHook();
    useWebGLWatchdog();

    const { runAsLoading } = useAppLoadingNotifications();

    const displayData = 'test';
    ...
    return {
      ...
      leftSideBar: ref(true), //ref(!root.$vuetify.breakpoint.mobile),
      messageCount,
      messageBadgeColor,
      layoutName,
      currentLayout,
